        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`relative px-4 py-2 rounded-full font-medium link-hover focus:outline-none focus:ring-0 ${pathname === item.href
                    ? 'text-amber-400'
                    : 'text-gray-300'
                  }`}
              >
          ...
          >
            <div className="w-6 h-6 flex flex-col justify-center items-center">
              <span className={`block w-5 h-0.5 bg-current transform transition-all duration-200 ${isMenuOpen ? 'rotate-45 translate-y-1.5' : '-translate-y-1'
                }`}></span>
              <span className={`block w-5 h-0.5 bg-current transform transition-all duration-200 ${isMenuOpen ? 'opacity-0' : 'opacity-100'
                }`}></span>
              <span className={`block w-5 h-0.5 bg-current transform transition-all duration-200 ${isMenuOpen ? '-rotate-45 -translate-y-1.5' : 'translate-y-1'
                }`}></span>
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-8">
              {navItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsMenuOpen(false)}
                  className={`text-xl md:text-2xl font-bold link-hover focus:outline-none focus:ring-0 min-h-[44px] flex items-center justify-center px-6 py-4 ${pathname === item.href
                      ? 'text-amber-400'
                      : 'text-white'
                    }`}
                >
